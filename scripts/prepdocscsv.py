...
parser = argparse.ArgumentParser(
    ...
    epilog="Example: prepdocscsv.py './data/*' --searchservice mysearch --index myindex -v"
)
...
if args.removeall:
    ...
    if not args.remove:
        create_search_index()
        print(f"Processing CSV files in folder: {args.files}")

        # List all files in the folder and filter for CSV files
        all_files = os.listdir(args.files)
        csv_files = [os.path.join(args.files, f) for f in all_files if f.endswith('.csv')]
